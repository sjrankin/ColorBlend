...
namespace Iro3.Data.ColorSpaces
{
    public partial class IroColorSpace
    {
        ...
        internal static void MakeUStandardColors ()
        {
            UStandardColors = new Dictionary<string, Color>();
            foreach (KeyValuePair<string, Color> KVP in StandardColors)
                UStandardColors.Add(KVP.Key.ToLower(), KVP.Value);
        }

        internal static bool UColorNameExists (string ColorName)
        {
            if (UStandardColors == null)
                MakeUStandardColors();
            if (string.IsNullOrEmpty(ColorName))
                return false;
            return UStandardColors.ContainsKey(ColorName);
        }

        internal static Color GetUColorValue(string ColorName)
        {
            if (!UColorNameExists(ColorName))
                return Colors.Transparent;
            return UStandardColors[ColorName];
        }

        /// <summary>
        /// Dictionary of standard colors based on <seealso cref="StandardColors"/> but with all color names normalized.
        /// </summary>
        internal static Dictionary<string, Color> UStandardColors = null;
        /// <summary>
        /// Dicationary of standard colors.
        /// </summary>
        internal static Dictionary<string, Color> StandardColors = new Dictionary<string, Color>()
        {
